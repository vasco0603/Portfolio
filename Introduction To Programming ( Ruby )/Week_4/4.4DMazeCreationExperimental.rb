...
class GameWindow < Gosu::Window
  ...
  def initialize
    ...
    while col < x_cell_count
      row = 0
      while row < y_cell_count
        if @columns[col][row].south == nil
          @columns[col][row].south = "0"
        end
        if @columns[col][row].west == nil
          @columns[col][row].west = "0"
        end
        if @columns[col][row].north == nil
          @columns[col][row].north = "0"
        end
        if @columns[col][row].east == nil
          @columns[col][row].east = "0"
        end
        puts "Cell x: " + col.to_s + ", y: " + row.to_s + " north:" + @columns[col][row].north + " south:" + @columns[col][row].south + " east:" +@columns[col][row].east + " west:" + @columns[col][row].west
        row += 1
      end
      puts "------------ End of Column ------------"
      col += 1
  ...
  #prints the coordinates and the neighbouring links
