...
class Food
  ...
  attr_accessor :x, :y, :type, :image, :vel_x, :vel_y, :angle, :x, :y, :score, :changing, :count, :original
   ...
   @all_food.each do |food|
      if !food.changing
        if Gosu.milliseconds / 100 % rand(1..1000) == 1
          food.changing = true
          #ask tutor more about this
          #jawabannya itu karna food.count - 1 kalo ga ganti, it's affecting the time turning to smoke
          food.count = 20 #number of timesteps to flash
